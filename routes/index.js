...
router.get('/dn', function (req, res, next) {
    res.render('dn', {title: 'Express'});
});
router.get('/dk', function (req, res, next) {
    res.render('dk', {title: 'Express'});
});

router.get('/qlu', upload.single('avatar'), function (req, res, next) {
    ...
    userConnect.find({}, function (error, users) {
        ...
        if (type == 'json') {
            ...
            res.render('qlu', {title: 'Quản lý User', users: users});
        }
    });
